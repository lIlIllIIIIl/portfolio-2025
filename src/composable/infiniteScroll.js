...
export default function infiniteScroll() {
  async function doSmth() {
    ...
    // parameter #01
    const spacing = 0.09 // spacing of the cards (stagger)
    ...
    const seamlessLoop = buildSeamlessLoop(cards, spacing),
      ...
      trigger = ScrollTrigger.create({
        ...
        // définit la taille du container du scroll
        end: '400vh',
        pin: '.homepage-container',
      })
    ...
    function buildSeamlessLoop(items, spacing) {
      let overlap = Math.ceil(1 / spacing), // number of EXTRA animations on either side of the start/end to accommodate the seamless looping
        ...
        // goNext -> Changer la valeur du "1.1" pour ajuster le scroll
        loopTime = (items.length + overlap) * spacing + 1.1, // the spot at the end where we loop back to the startTime
